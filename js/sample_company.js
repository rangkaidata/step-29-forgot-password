...
function SampleCompany(){
	...
	function init(){
		...
		function tes(paket){
			...
			html+='</table>';
			html+='<p><i>&#10020 Daily Planet sudah diakuisisi oleh Wayne Enterprises sebuah entitas perusahaan milik Bruce Wayne dari Gotham city.</i></p>';
	...
	function createCompany(paket){
		...
		if (paket.err===0){
			...
			loadXHR(url2+"open.php",obj,openCompany); 
		}
	}
	...
	function createAccount(paket){
		...
		if (paket.err==0){
			...
			if (account_proses===account_count){
				...
				loadXHR(url4+'import.php',obj,importJournal);
			}
		}
		else{
			proses_123+='['+account_proses+'] account: '+paket.msg+'<br>';
