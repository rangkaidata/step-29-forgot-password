...
function Journal(login_){
	...
	function refreshTable(isi){
		...
		for (let i=0;i<panjang;i++){
			html+='<tr>'
				...
				+'<td><button id="btn_remove" onclick="objModul.removeRow('+i+')"></button></td>'
				+'</tr>';
	...
	function bacaData(isi){
		...
		loadXHR(url+'import.php',obj,testData);
	}
	...
	function importExecute(){
		...
		for (var x in paket.data) {
			...
			paket.data[x].journal_date
			paket.data[x].journal_notes*/
			...
			function importDataMessage(paket){
				...
				document.getElementById("msgImport").innerHTML="<h4>Message Proccess:</h4>"+oMsg+'<h4>End Proccess.</h4>';
			}
			...
			for (var x in paket.data) {
				...
				for (var y in paket.data[x].journal_detail)	{
					html+='<tr>'
						...
						+'<td>'+paket.data[x].journal_detail[y].debit+'</td>'
						+'<td>'+paket.data[x].journal_detail[y].credit+'</td>'
